...
| 0x07 | CMD_QUERY_VERSION | Server → Device | 连接 | 查询当前运行版本 |
|           |                   |                 |      |                    |
|           |                   |                 |      |                    |
|           |                   |                 |      |                    |
...
### 3.4 连接阶段版本查询 (Server → Device)（命令码 0x07）

上位机在下载前先查询设备当前运行版本，决定是否继续升级。
...
```c
typedef struct {
    uint32_t error_code;            // 0: 查询成功
    uint8_t  fw_version_major;      // 当前运行版本
    uint8_t  fw_version_minor;
    uint8_t  fw_version_patch;
} Query_Version_Resp_t;
```
...
### 3.6 通用错误码补充 (Error Code Definitions)
